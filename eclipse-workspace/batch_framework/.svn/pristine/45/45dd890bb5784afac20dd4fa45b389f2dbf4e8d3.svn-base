...
public class BaseOutputDataProcessValve4Detail implements Valve 
{
			...
			throws DataPipeException
	{
		synchronized (config)
		{
			String controlFlag = ip_pipeData
					.getPipeFieldStringValue("controlFlag");
			if (controlFlag == null || "".endsWith(controlFlag)
					|| !controlFlag.toUpperCase().matches("[A-Z](,[A-Z])*"))
			{
				throw new DataPipeException(
						" [ Undefined controlFlag format :'"
								+ controlFlag
								+ "' in rowData! Prefer format: '[A-Z](,[A-Z])*' ] ");
			}
			docCnt += controlFlag.split(",").length;
