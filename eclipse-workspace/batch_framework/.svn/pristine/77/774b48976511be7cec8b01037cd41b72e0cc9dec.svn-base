...
public class BaseOutputDataProcessValve4Mutl implements Valve 
{
			...
			throws DataPipeException
	{
		synchronized (config)
		{
			if(null!=ip_pipeData.getPipeFieldStringValue("controlFlag")&&ip_pipeData.getPipeFieldStringValue("controlFlag").equals("Y")){
				docCnt+=2;
			}else{
				docCnt++;
			}			
