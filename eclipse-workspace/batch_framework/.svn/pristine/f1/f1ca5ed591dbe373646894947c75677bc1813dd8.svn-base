...
public abstract class BackupBaseService implements BackupService
{
			...
			throws BatchJobException
	{
		...
		try
		{
			PatternUtil loc_sqlJob = new PatternUtil(ip_backupParameter
					.get("acDate"));
			// parameter
			...
			{
				loc_conStr = " WHERE "
						+ loc_sqlJob.patternReplace(loc_sqlParameter.dealSqlStr(),
								loc_conStr);
			}
		} catch (Exception e)
		{
			...
			String ip_acDate) throws BatchJobException
	{
		try
		{
			String loc_deleteFileName = new File(ip_targetName.replaceAll(
					PatternUtil.PATTERN_STRING, "")).getName();
			if (ip_keepFileDays != null && !ip_keepFileDays.equals("0"))
			{
				int loc_index = 0;
				Matcher loc_m = PatternUtil.PATTERN.matcher(ip_targetName);
				...
				PatternUtil loc_jobUtil = new PatternUtil(DateUtil.addAcDays(
					...
					if (loc_file1.isDirectory())
					{
						...
						for (File loc_f : loc_files)
						{
							this.deleteFile(loc_backupName + loc_f.getName()
									+ loc_jobUtil.patternReplace(null, loc_endName),
									loc_deleteFileName);
						}
					}
				} else
				{
					this.deleteFile(loc_jobUtil.patternReplace(null, ip_targetName),
							loc_deleteFileName);
				}
			...
			Map<String, String> ip_backupParameter) throws BatchJobException
	{
		...
		try
		{
			...
			PatternUtil loc_sqlJob = new PatternUtil(ip_backupParameter
					.get("acDate"));
			ip_fileName = loc_sqlJob.patternReplace(null, ip_fileName);
