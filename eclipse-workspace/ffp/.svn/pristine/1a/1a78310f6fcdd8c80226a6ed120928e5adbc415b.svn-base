...
(function ($) {
	...
	function calElement(sYear, sMonth, sDay, week, lYear, lMonth, lDay, isLeap, cYear, cMonth, cDay) {
		...
		{
			var obj = holidayList[i];
			if(sYear == obj.year && sMonth == obj.month && sDay == obj.day)
			{
				this.isHoliday = 'Y';
			}
		}
	...
	function Calendar(y, m) {
		...
		//备忘信息
		for (i in sMemo){
			if (sMemo[i].match(/^(\d{2})(\d{2})([\s\*])(.+)$/))
				if (Number(RegExp.$1) == (m + 1)) {
					this[Number(RegExp.$2) - 1].calMemo += RegExp.$4 + ' ';
					if (RegExp.$3 == '*')
						this[Number(RegExp.$2) - 1].color = 'red';
				}
		}
	...
	function show(target) {
		...
		$(target).find('.calendar-title span').html(opts.months[opts.month - 1] + ' ' + opts.year + ' ' + cyclical(opts.year - 1900 + 36) + '年 【' + Animals[(opts.year - 4) % 12] + '年】');
		...
		for (var i = 0; i < weeks.length; i++) {
			...
			for (var j = 0; j < week.length; j++) {
				var day = week[j];
				var dayHtml = '<font size="4" face="Arial">' + day[2];
				var info = null;
				if (opts.month == day[1]) {
					info = currentCa[day[2] - 1];
					if (info.color) {
						var color = 'color="' + info.color + '"';
						dayHtml = '<font size="4" face="Arial" ' + color + '>' + day[2];
					}
					dayHtml += '</font>';
					if (opts.lunarDay) {
						dayHtml += "<br/>";
				...
				if (info && (info.isHoliday)) {
					day.addClass('calendar-holiday-sign');
				}
				day.hover(function (e) {
					...
					if (inf) {
						...
						if (inf.calMemo) {
							detail.append('<div class="calendar-memo">' + inf.calMemo + '</div>');
						}
					} else {
						detail.hide();
					}
	...
	//增加样式
	$('head').append('<style>.fullcalendar-day {border-bottom: 1px #c8cacc;} '
		 + '.calendar-hover{border: 1px solid red;}.fullcalendar-body th{padding: 5px;height:20px;}'
		 + '.fullcalendar-detail{border-radius:5px;position: absolute; z-index: 10; background-color: #0d0d0d; display: none; opacity: 0.8; padding: 5px; text-align: right;filter: Alpha(opacity=80);width:150px;}'
		 + '.fullcalendar-detail div{margin-top: 3px;font-size: 12px;padding: 2px;}'
		 + '.fullcalendar-detail .calendar-memo{background:#86b1ce;margin-top: 3px;font-size: 12px;padding: 2px;}'
		 + '.fullcalendar-detail .lunarFestival{color: red;}.fullcalendar-detail .solarFestival{color: cyan;} .calendar-selected {border: 1px solid red;} .holiday-selected {border: 1px solid red; background: #edfac8}');
