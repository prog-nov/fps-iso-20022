...
public class UserService implements IUserService
{
	...
	{
		checkRoleLimit(user);
		checkDeleteAdministrator(new String[] { user.getUserId() });
	 ...
	 */
	@Override
	public int sDelete(String[] userIds, String operId)
	{
		checkDeleteAdministrator(userIds);
		checkDeleteYou(userIds, operId);
	...
	{
		checkDeleteAdministrator(new String[] { userId });
	...
	@Override
	public int sUpdateUserStatus(String userId, String operId, boolean status)
	{
		checkUpdateUserStatus(userId, operId);
		UserForm user = new UserForm();
	...
	private void checkDeleteYou(String[] userIds, String oper)
	{
		for (String userId : userIds)
		{
			if (oper.equals(userId))
			{
				Throw.throwRuntimeException(ExceptionCodes.AP020104);
			}

		}
	}

	private void checkUpdateUserStatus(String userId, String operId)
	{

		if (operId.equals(userId))
		{
			Throw.throwRuntimeException(ExceptionCodes.AP020103);
		}
