...
public class FFPApacheMqSenderAgent extends FFPMqSenderAgent
{
	...
	public void init()
	{
		...
		try
		{
			...
			this.jmsTemplate.setDefaultDestinationName(config.getDefaultDestinationQueueName());
	...
	public FFPSendMessageResp sendAcknowledge(String message, Map<String, Object> propertyMap, String priority)
	{
		...
		{
			destinationName = config.getOutwardAcknowledgeHighPriorityQueueName().getQueueName();
		} else if (FFPConstants.MQ_LEVEL_PRIORITY_MEDIUM.equals(priority))
		{
			destinationName = config.getOutwardAcknowledgeMediumPriorityQueueName().getQueueName();
		}
	...
	public FFPSendMessageResp sendRequest(String message, Map<String, Object> propertyMap, String priority)
	{
		...
		if (FFPConstants.MQ_LEVEL_PRIORITY_HIGH.equals(priority))
		{
			destinationName = config.getOutwardRequestHighPriorityQueueName().getQueueName();
		} else if (FFPConstants.MQ_LEVEL_PRIORITY_MEDIUM.equals(priority))
		{
			destinationName = config.getOutwardRequestMediumPriorityQueueName().getQueueName();
		}
