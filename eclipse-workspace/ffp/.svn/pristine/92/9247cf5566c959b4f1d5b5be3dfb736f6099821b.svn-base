...
public class FFPMsgBaseParticipantMessage
{
	...
	protected String responseMsg;    //MsgResult
	protected String responseMsgCode;  //MsgCode
	protected Date respMsgCreateTs;
	...
	public HEAD marshalMsgReqHead()
	{
		...
		head.setTransactionDate(FFPDateUtils.getDateStr(this.createTs, FFPDateUtils.INT_DATE_FORMAT));
		head.setTransactionTime(FFPDateUtils.getTimeStr(FFPDateUtils.getTime(this.createTs), FFPDateUtils.INT_TIME_FORMAT));
	...
	public void unmarshalResponseMsg(String ip_responseMsg) throws Exception
	{
		try
		{
			com.forms.ffp.adaptor.jaxb.participant.response.ROOT root = FFPParticipantMessageConverter.parseXml2ReponseObject(ip_responseMsg);
			com.forms.ffp.adaptor.jaxb.participant.response.head.HEAD head = root.getHEAD();
			this.resRefNo = head.getResponseRefno();
			this.respMsgCreateTs = new Date();
			...
			unmarshalResponseBody(root.getBODY());
		} catch (Exception e)
		{
			throw e;
		}
	...
	public void unmarshalResponseBody(com.forms.ffp.adaptor.jaxb.participant.response.BODY body)
	{
		return;
	}
	...
	public Date getRespMsgCreateTs() {
		return respMsgCreateTs;
	}

	public void setRespMsgCreateTs(Date respMsgCreateTs) {
		this.respMsgCreateTs = respMsgCreateTs;
	}
