		...
		Runnable run = new Runnable()
		{
			...
			public void run()
			{
				for (DefaultMessageListenerContainer container : FFPApacheMqListenerAgent.this.listenerContainer)
				{
					...
					{
						FFPApacheMqListenerAgent.logger.info("[{}] Starting listener... Destination: [{}]",
								new Object[] { FFPApacheMqListenerAgent.this.config.getConnectorName(), container.getDestinationName() });
					}
				...
				if (FFPApacheMqListenerAgent.logger.isInfoEnabled())
				{
					FFPApacheMqListenerAgent.logger.info("[{}] All Listeners started.", FFPApacheMqListenerAgent.this.config.getConnectorName());
				}
			}
		};
