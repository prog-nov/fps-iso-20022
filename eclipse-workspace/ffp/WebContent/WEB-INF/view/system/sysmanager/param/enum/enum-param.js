	...
	Page.create(function($){
		var me = this,
			...
			editableRender = function(v){
				return v === '1' ? locale.system.sysmanager.param.enum.yes : locale.system.sysmanager.param.enum.no;
			};
		...
		return {
			init :  function(){
				grid.datagrid({
					...
					columns : [ [ 
					    {checkbox : true},
			            {field : 'paramCode', title : locale.system.sysmanager.param.enum.ParametersCode, align:'center'},
			            {field : 'paramName', title : locale.system.sysmanager.param.enum.parameterName, align:'left'}, 
			            {field : 'paramGroup', title : locale.system.sysmanager.param.enum.section, align:'left', width : 100, sortable : true},
						{field : 'paramAttr', title : locale.system.sysmanager.param.enum.parameterCharacteristics, align:'left'},
						{field : 'editable', title : locale.system.sysmanager.param.enum.Canedit, align:'left', formatter: editableRender},
						{field : 'seqno', title : locale.system.sysmanager.param.enum.orderNumber, align:'left'},
						{field : 'des', title : locale.system.sysmanager.param.enum.describe, align:'left'}
			        ] ]
				});
			...
			gotoAdd : function (){
				dialog.dialog({    
				    title: locale.system.sysmanager.param.enum.definition,    
				    height:400,
				    width:380,
				    href : 'add.page',
				    onLoad : function(){
			...
			gotoUpdate : function (data){
				dialog.dialog({    
				    title: locale.system.sysmanager.param.enum.compile,    
				    height:400,
				    width:380,
				    href : 'edit.page',
				    onLoad : function(){
			...
			gotoUpdateData : function (data){
				if(data && data.paramAttr === 'LIST'){
					dialog.dialog({    
					    title: locale.system.sysmanager.param.enum.detail,    
					    height:500,
					    width:640,
					    maximized:true,
					    ...
					    onLoad : function(){
					    	...
					    	listGrid.datagrid({
								...
								columns : [ [ 
						            {field : 'dataCode', title : locale.system.sysmanager.param.enum.dataCode, align:'left', editor:'text'}, 
						            {field : 'dataText', title : locale.system.sysmanager.param.enum.TextData, align:'left', editor:'text'},
									{field : 'dataParam', title : locale.system.sysmanager.param.enum.DataParameter, align:'left', editor:'text'},
									{field : 'seqno', title : locale.system.sysmanager.param.enum.orderNumber, align:'left', editor:'text'},
									{field : 'des', title : locale.system.sysmanager.param.enum.describe, align:'left', editor:'text'}
						        ] ]
							});
					    },
					    buttons : [ {
							text : locale.button.save, size : 'large', handler : me.doUpdateListDetail
						}, {
							text : locale.button.cancel, size : 'large', handler : me.closeDialog
						} ]
					});
					...
					dialog.dialog({    
					    title: locale.system.sysmanager.param.enum.treeParameter,    
					    height:500,
					    width:640,
					    maximized:true,
					    ...
					    buttons : [ {
							text : locale.button.save, size : 'large', handler : me.doUpdateTreeDetail
						}, {
							text : locale.button.cancel, size : 'large', handler : me.closeDialog
						} ]
					});
				...
				if (me.endEditing()){
					...
					if(changes.length>0){
						...
						$b.Submit.ajaxSubmit(apis.saveListData, params, function(rs){
							$b.Msg.alert(locale.system.sysmanager.param.enum.saveSuccessfully, function(){
								listGrid.datagrid('load');
							});
						});
					}else{
						$b.Msg.warning(locale.system.sysmanager.param.enum.noContent);
					}
				}
			},			
			...
			doDelete : function (rows){
				$b.Msg.confirm(locale.system.sysmanager.param.enum.remove1+rows.length+locale.system.sysmanager.param.enum.remove2, function(){
					var paramCodes = [];
					$.each(rows, function(i, n){
			...
			deleteCallback : function (rs){
				$b.Msg.alert(locale.system.sysmanager.param.enum.removeS + rs + locale.system.sysmanager.param.enum.removeS2);
				me.doQuery();
			},
			/**
			 * 执行查询
			...
			doInsertTreeNode : function (){
				$b.Submit.ajaxSubmitForm(treeForm, {url : apis.insertTreeNode}, function(rs){
					...
					$b.Msg.alert(locale.system.sysmanager.param.enum.succeed1);
				});
			},
			/**
			 * 执行更新树节点
			...
			doUpdateTreeNode : function (){
				$b.Submit.ajaxSubmitForm(treeForm, {url : apis.updateTreeNode}, function(){
					...
					$b.Msg.alert(locale.system.sysmanager.param.enum.succeed2);
				});
			},
			/**
			 * 移动树形结点
			...
			moveTreeNode : function(options){
	    		...
	    		$b.Submit.ajaxSubmit(url, params, function(){
	    			...
	    			$b.Msg.alert(locale.system.sysmanager.param.enum.node+" <font color='red'>"+snode.text+"</font> "+locale.system.sysmanager.param.enum.DragTheSuccess);
			...
			_bindpopupMenuEvent : function(){
				...
				popupMenu.find(".menuAdd").on("click", function(){
					...
					$b.Submit.ajaxSubmit(apis.findTreeNode, param, function(data){
		   			...
		   			$b.Msg.confirm(locale.system.sysmanager.param.enum.sureToDelete+"<font color='red'> "+node.text+(count > 0 ? locale.system.sysmanager.param.enum.sureToDelete2 : "")+" </font>？", function(){
		   				$b.Submit.ajaxSubmit(apis.deleteTreeNode, {dataCode : node.id, paramCode: node.attributes.paramCode}, function(){
		   					...
		   					$b.Msg.alert(locale.system.sysmanager.param.enum.succeedRemove+'<font color="red">' + node.text + '</font>');
		   				});
		   			});
				});
			}
