			...
			if (key.matches(connectorKey))
			{
				try
				{
					...
					if (FFPConstants.CONNECTTOR_TYPE_MQ_APACHEMQ.equals(connectorType) || FFPConstants.CONNECTTOR_TYPE_MQ_WEBPHEREMQ.equals(connectorType))
					{
						...
						FFPQueueConfig queueConfig1 = new FFPQueueConfig();
						String str1 = loc_connectorPro.get(value + ".send.req." + FFPConstants.MQ_LEVEL_PRIORITY_HIGH);
						if(str1 == null)
							_logger.error(value + ".send.req." + FFPConstants.MQ_LEVEL_PRIORITY_HIGH + " must be setup");
						queueConfig1.setQueueName(str1);
						mqConfig.setOutwardRequestHighPriorityQueueName(queueConfig1);
						
						FFPQueueConfig queueConfig2 = new FFPQueueConfig();
						String str2 = loc_connectorPro.get(value + ".send.req." + FFPConstants.MQ_LEVEL_PRIORITY_MEDIUM);
						if(str2 == null)
							_logger.error(value + ".send.req." + FFPConstants.MQ_LEVEL_PRIORITY_MEDIUM + " must be setup");
						queueConfig2.setQueueName(str2);
						mqConfig.setOutwardRequestMediumPriorityQueueName(queueConfig2);
						
						FFPQueueConfig queueConfig3 = new FFPQueueConfig();
						String str3 = loc_connectorPro.get(value + ".send.ack." + FFPConstants.MQ_LEVEL_PRIORITY_HIGH);
						if(str3 == null)
							_logger.error(value + ".send.ack." + FFPConstants.MQ_LEVEL_PRIORITY_HIGH + " must be setup");
						queueConfig3.setQueueName(str3);
						mqConfig.setOutwardAcknowledgeHighPriorityQueueName(queueConfig3);
						
						FFPQueueConfig queueConfig4 = new FFPQueueConfig();
						String str4 = loc_connectorPro.get(value + ".send.ack." + FFPConstants.MQ_LEVEL_PRIORITY_MEDIUM);
						if(str4 == null)
							_logger.error(value + ".send.ack." + FFPConstants.MQ_LEVEL_PRIORITY_MEDIUM + " must be setup");
						...
						if(queuemgrConfig.getSubscribeInward())
						{
							...
							for (String queuekey : loc_connectorPro.keySet())
							{
								if (queuekey.matches(receivequeueRegex))
								{
									...
									queueConfig.setPriority(queuekey.substring(queuekey.lastIndexOf(".") + 1));
