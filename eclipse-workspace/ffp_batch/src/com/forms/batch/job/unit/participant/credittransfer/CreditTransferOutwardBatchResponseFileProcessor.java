...
public class CreditTransferOutwardBatchResponseFileProcessor extends BatchBaseJob
{
		...
		try
		{
			...
			if (queryMapList != null && queryMapList.size() > 0)
			{
				for (Map<String, Object> map : queryMapList)
				{
					...
					toAgent.setOrgnlTxId(map.get("ORG_TRANSACTION_ID") != null ? (String) (map.get("ORG_TRANSACTION_ID")) : null);
					toAgent.setOrgnlEndToEndId(map.get("ORG_END_TO_END_ID") != null ? (String) (map.get("ORG_END_TO_END_ID")) : null);
					...
					toAgent.setClrSysRef(map.get("TX_CLR_SYS_REF") != null ? (String)(map.get("TX_CLR_SYS_REF")) : null);
					
					toAgent.setTempBatchId((int)map.get("ID"));//Temp table records id
	...
	private void processResponseTxData(Connection conn, List<FFPVO_Pacs002_TxInfAndSts> tepDataList)
	{
		...
		for (FFPVO_Pacs002_TxInfAndSts tx_pacs002 : tepDataList)
		{
			try
			{
				//Add parameters for avoid get return or refund data
				String sql_query = "SELECT JNL.JNL_NO, JNL.SRC_REF_NM, ACTION.MSG_ID FROM TB_TX_JNL JNL JOIN TB_TX_JNLACTION ACTION ON JNL.JNL_NO = ACTION.JNL_NO "
						+ 	 	   "WHERE JNL.TRANSACTION_ID = ? AND JNL.END_TO_END_ID = ? AND JNL.FPS_REF_NUM IS NULL AND JNL.TX_CODE = ? AND ACTION.MSG_TYPE = ?";
				
				List<Object> listData = new ArrayList<Object>();
				listData.add(tx_pacs002.getOrgnlTxId());
				listData.add(tx_pacs002.getOrgnlEndToEndId());
				listData.add(FFPConstantsTxJnl.TX_CODE.TX_CODE_P100.getCode());
				listData.add(FFPJaxbConstants.JAXB_MSG_TYPE_FFPCTO01);
				...
				txJnl.setSrcRefNm(srcRefNo);
				p100.setTxJnl(txJnl);
				tx_pacs002.setP100Jb(p100);
				...
				insertAcitonFlow(conn, jnlNo, resRefNo, FFPConstants.MSG_DIRECTION_OUTWARD, FFPConstants.MSG_CODE_AGENT, FFPJaxbConstants.JAXB_MSG_TYPE_FFPCTO01,
				...
				if(response != null)
				{
					...
					if(resp.isTimeOut())
					{
						//not update TB_TX_JNL cause pacs002 is the last status of transaction
						//If timeout, update the status and then resent will roll it back
