	...
	Page.create(function($){
		...
		return {
			init :  function(){
				...
				//初始化下拉列表
				me.setComboFields($('#comboSearchForm'));
			    me.setOrganField($('#comboSearchForm'));
...
//					url : apis.list,  接口地址
					...
					idField : 'id',
					pagination : true,
					fitColumns : true,//自动适应宽度不出现横向滚动条，适合内容少的情况
					rowTool:{positionRight : "5%"},
					rowToolButtons:[
				...
				$(".Action").find("a.SearchBtn").on("click", me.doQuery)
					   ...
					   .end().find("a.EditBtn").on("click", function(){
					    ...
					    }).end().find("a.MoreSearchBtn").on("click",me.gotoMoreSearch);
			 ...
			 * 进入更多查询
			 */
			gotoMoreSearch : function (){
				var options={    
    	   			    title: locale.title.search,    
    				    height:500,
    				    width:900,
    				    maximized:true,
    	   			    buttons:[{
    	   			    	text :locale.button.submitSearch, size : 'large', handler : me.doQuery
    	   			    }]
				}
				
    			if(searchDialog.children().length==0){
    				options.href='demo2-search-more.page';
    				$('#searchDialog').dialog(options);
    			}else{
