		...
		return {
			init :  function(){	
				var i,
					...
					menuLocales,
					columns;
					columns = [ {title:'菜单',field:'menuName',width:10,
				...
				tree.treegrid({
					...
					onAfterEdit : function(rowData){
						...
						if(index === bengingArr[len]){
							if(editChange){
								$b.Msg.confirm("数据已经更改，是否提交已修改数据？", function(){
			...
			doSave : function(){
				...
				if(changesLen > 0){
					...
					$b.Msg.warning('请先修改表格中的数据再进行保存操作。');
				};
			},
			/**
			 * 	点击编辑按钮，编辑选中行
