	...
	Page.create(function($){
		...
		return {
			init :  function(){
				grid.datagrid({
					...
					columns : [ [ 
						...
						{field : 'roleName', title : '角色名称', align:'left', width: 200},
			...
			doDelete : function (rows){
				$b.Msg.confirm('您确定要删除选择的'+rows.length+'个角色吗？', function(){
					var roleIds = [];
			...
			loadRoleAllot : function(container, data){
				...
				grid.datagrid({
					...
					columns:[[
						...
						{field:'roleAllotName', title: '角色(分配)名称', fit: true, width: 250}
					]],
			...
			loadTree : function (container, data, permType){
				...
				tree.treegrid({
					...
					columns:[[
			          ...
			          {title:'菜单显示属性',field:'permAttr', width: 100,
						  formatter: function(v, row){
							  if(row.leaf){
								  return v === '1' ? '显示可操作' : (v === '2' ? '显示不可操作' : '');
							  }else{
								  row.permAttr = '';
								  return '';
							  }
						  ...
						  editor : {
				        	  ...
				        	  options:{
				        		  valueField:'id',
				        		  textField:'text',
				        		  data : [{id : '' , text : '--请选择--'}, {id : '1', text : '显示可操作'},{id : '2', text : '显示不可操作'}]
				        	  }
			          	  }
			          }    
			      	]],
