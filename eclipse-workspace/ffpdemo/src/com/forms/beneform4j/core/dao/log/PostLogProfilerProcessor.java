...
/**
 * Post processes an existing sqltiming log, and creates a profiling report from it. Name of log
 * file is passed in on the command line as the only argument.
 *
 * Assumptions:
 *
 * 1. Each sql statement in the log is separated by a blank line. 2. Each sql statement is
 * terminated with the timing string "{executed in N msec}" where N is the number of milliseconds
 * that the sql executed in.
 *
 */
...
public class PostLogProfilerProcessor {
     ...
     *
     * @param args command line arguments. Expects one argument, the name of the file to post
     *        process.
