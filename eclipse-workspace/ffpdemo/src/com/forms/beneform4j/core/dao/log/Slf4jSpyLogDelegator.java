...
public class Slf4jSpyLogDelegator implements SpyLogDelegator {
    ...
    /**
     * Logger that shows connection open and close events as well as current number of open
     * connections.
     */
    ...
    /**
     * Called when a JDBC method from a Connection, Statement, PreparedStatement, CallableStatement
     * or ResultSet returns.
     *
     ...
     *        representation for Object. Return types this will be null for void return types.
     */
     ...
     * the stack trace information from the point just before we got it
     * (com.forms.beneform4j.core.dao.log)
     *
     * if the optional log4jdbc.debug.stack.prefix system property is defined then the last call
     * point from an application is shown in the debug trace output, instead of the last direct
     * caller into log4jdbc
     *
    ...
    private static String getDebugInfo() {
        ...
        if (stackTrace != null) {
            ...
            /**
             * The DumpFullDebugStackTrace option is useful in some situations when we want to see
             * the full stack trace in the debug info- watch out though as this will make the logs
             * HUGE!
             */
            if (DriverSpy.DumpFullDebugStackTrace) {
                ...
                for (int i = 0; i < stackTrace.length; i++) {
                    ...
                    if (!className.startsWith("com.forms.beneform4j.core.dao.log")) {
                        if (first) {
                            first = false;
                        } else {
                            dump.append("  ");
